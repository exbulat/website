...
<div class="container">
    <div class="row mb-4">
        <div class="col-md-8">
            ...
            <p class="lead text-muted">Результаты поиска по запросу: "{{ $query }}"</p>
        </div>
        <div class="col-md-4">
            <form action="{{ route('public.surveys.search') }}" method="GET" class="mt-3">
                <div class="input-group">
                    <input type="text" name="q" class="form-control" placeholder="Поиск опросов..." value="{{ $query }}">
    ...
    @if($surveys->isEmpty())
        <div class="alert alert-info">
            <i class="fas fa-info-circle me-2"></i>По запросу "{{ $query }}" не найдено опросов.
        </div>
        <div class="text-center mt-4">
            <a href="{{ route('public.surveys.index') }}" class="btn btn-primary">
                <i class="fas fa-arrow-left me-1"></i>Вернуться к списку опросов
            </a>
        </div>
    @else
        <div class="mb-3">
            <p>Найдено опросов: {{ $surveys->total() }}</p>
        </div>
        <div class="row">
